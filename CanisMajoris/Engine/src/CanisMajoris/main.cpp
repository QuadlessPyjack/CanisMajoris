...
int main(int argc, char* argv[])
{
	...
	//EdgePool = testOBJ->ExtractEdgeData(EDGE_COUNT, VertexPool);
	...
	cout << "Mesh[" << MeshPool.GetMesh(1)->GetID().c_str() << "] World Coordinates: " << MeshPool.GetMesh(1)->Location() << " Camera-Relative: " << WorldToCameraCoords(MeshPool.GetMesh(1)->Location()) << endl;

	Uint8 *keystates = SDL_GetKeyState(NULL);
	bool shouldQuit = false;
	SDL_Event event;
	//MeshPool.GetMesh(1)->Rotate(Vector3(400, 300, 0), Vector3(0.0f, 0.9f, 0.0f));
	for (int i = 0; i < MeshPool.length(); ++i)
	{
	...
	while (!shouldQuit)
	{
		...
		for (int i = 0; i < MeshPool.length(); ++i)
		{
			MeshPool.GetMesh(i)->Translate(Vector3(10.0f, 3.5f, 0.7f));
			if (i % 2 == 0)
			{
				MeshPool.GetMesh(i)->Translate(Vector3(12.8f, 1.2f, 0.0f));
				//MeshPool.GetMesh(i)->Scale(0.995f);
				//MeshPool.GetMesh(i)->Rotate(Vector3(0,0,0), Vector3(0.0f,0.1f,0.0f));
				MeshPool.GetMesh(i)->Draw();
			}
			else
				if (i % 3 == 0)
				{
					//MeshPool.GetMesh(i)->Translate(Vector3(-60.2f, -1.2f, 10.0f));
				//MeshPool.GetMesh(i)->Rotate(Vector3(0, 0, 0), Vector3(0.0f, 0.1f, 0.0f));
					//MeshPool.GetMesh(i)->Scale(1.002f);
					MeshPool.GetMesh(i)->Draw();
				}
				else
				{
					MeshPool.GetMesh(i)->Translate(Vector3(5.2f, -1.2f, 0.0f));
					//MeshPool.GetMesh(i)->Scale(0.998f);
					//MeshPool.GetMesh(i)->Rotate(Vector3(0, 0, 0), Vector3(0.0f, 500.1f, 0.0f));
					MeshPool.GetMesh(i)->Draw();
				}
			//TrianglePool[i]->Translate(Vector3(0.2f, 0.0f, 0.0f));
			//TrianglePool[i]->Draw();
			std::cout << "FRAME TICK\n";

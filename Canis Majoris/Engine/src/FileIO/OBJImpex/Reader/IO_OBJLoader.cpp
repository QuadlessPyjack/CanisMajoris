	...
	namespace IO
	{
		...
		MeshContainer ModelFile::ExtractMeshData(std::vector<Renderer::CoreUtils::Vertex*>& vertexPoolRef)
		{
			...
			if (m_input && *m_input)
			{
				//std::cout << "FILE DUMP: " << m_input->rdbuf()<<'\n';
				std::string owningMesh = "ROOT";
				MeshData.AddMesh(new Core::Renderer::CoreUtils::Mesh(owningMesh));
						...
						if (line[0] == 'v')
						{
							...
							while (line2 >> raw_value)
							{
								if (raw_value.compare("v") != 0)
								{
									...
									//Begin Glorious Hack
									Core::Renderer::CoreUtils::Vertex* vertex[1];
									vertex[0] = new Core::Renderer::CoreUtils::Vertex(raw_coordinates);
